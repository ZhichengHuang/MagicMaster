...
from magicmaster.utils.typing import ForwardInputs
from .tome_utils import (add_tome_cfg_hook, build_magicmaster_tomesd_block,
                         build_magicmaster_wrapper_tomesd_block, isinstance_str)
...
def xformers_is_enable(verbose: bool = False) -> bool:
    ...
    """
    from magicmaster.utils import try_import
...
def set_tomesd(model: torch.nn.Module,
    ...
    is_magicmaster = isinstance_str(model, 'StableDiffusion') or isinstance_str(
        model, 'BaseModel')
        ...
        if isinstance_str(module, 'BasicTransformerBlock'):
            ...
            if is_magicmaster:
                if is_wrapper is None:
                    ...
                    make_tome_block_fn = build_magicmaster_tomesd_block
                elif is_wrapper:
                    make_tome_block_fn = build_magicmaster_wrapper_tomesd_block
            else:
                raise TypeError(
...
def remove_tomesd(model: torch.nn.Module):
    ...
    """
    # For magicmaster Stable Diffusion models
