...
from magicmaster.structures import DataSample
from magicmaster.utils.typing import  SampleList
...
class MagVitVQGAN(BaseGan):
    ...
    def get_last_layer(self):
        return self.generator.module.decoder.conv_out.conv.weight
    ...
    def train_generator(self, inputs: Tensor, data_samples: DataSample,
        ...
        recon_video, codes, indice = self.generator(video, cond=None,
        ...
        parsed_loss, log_vars = self.gen_loss(disc_pred_fake, recon_video, video,frame_indices)
    ...
    def train_discriminator(self, inputs: Tensor, data_samples: DataSample,
        ...
        with torch.no_grad():
            recon_video,codes,indice = self.generator(video, cond= None, video_contains_first_frame=video_contains_first_frame)
    ...
    def train_step(self, data: dict,
        ...
        discriminator are updated alternatively. In magicmaster's design,
...
class MagVitAFVQGAN(BaseGan):
    ...
    def disc_loss(self, disc_pred_fake: Tensor, disc_pred_real: Tensor,real: Tensor) -> Tuple:
        ...
        losses_dict['loss_disc_real'] = torch.mean(F.relu(1. - disc_pred_real)) *0.5
        losses_dict['loss_disc_fake'] = torch.mean(F.relu(1. + disc_pred_fake)) *0.5
        losses_dict['disc_val_sum']=losses_dict['loss_disc_real'].detach()+losses_dict['loss_disc_fake'].detach()
    ...
    def gen_loss(self, disc_pred_fake: Tensor, recon_video: Tensor, video: Tensor ) -> Tuple:
        """Get gen loss. StyleGANv2 use the non-saturating loss and generator
        path regularization to train the generator.
        ...
        input_vgg_input = rearrange(video,'b c t h w -> (b t) c h w')
        recon_vgg_input = rearrange(recon_video, 'b c t h w -> (b t) c h w')


        p_loss = self.perceptual_loss(input_vgg_input.contiguous(), recon_vgg_input.contiguous())
        p_loss = p_loss.view(b,t)
        ...
        losses_dict['loss_gen']=d_weight*g_loss
        losses_dict['rec_value'] = rec_loss
        losses_dict['d_weight']=d_weight
    ...
    def train_generator(self, inputs: Tensor, data_samples: DataSample,
        ...
        # frame_indices = torch.randn((batch, frames)).topk(1, dim=-1).indices
        # recon_video_frames = pick_video_frame(recon_video, frame_indices)
        disc_pred_fake = self.discriminator(recon_video) # fake_logits




        parsed_loss, log_vars = self.gen_loss(disc_pred_fake, recon_video, video)
